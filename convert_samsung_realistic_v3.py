...
# Expanded Camera lenses box: x from 146 to 260 (expanded further right to prevent sensor cutoffs), y from 33 to 196.
...
for x in range(width):
    for y in range(height):
        ...
        if 143 <= x <= 357 and 31 <= y <= 468:
            ...
            if not is_border:
                is_camera_box = (146 <= x <= 260 and 33 <= y <= 196)
...
for y in range(33, 197):
    ...
    pixels[257, y] = c_highlight
    pixels[258, y] = c_mid
    pixels[259, y] = c_shadow
    pixels[260, y] = c_deep
...
if bbox:
    ...
    print("Successfully generated Samsung Ultra template with wide camera box!")
else:
