        ...
        if 143 <= x <= 357 and 31 <= y <= 468:
            ...
            if not is_border:
                ...
                if not is_camera_box:
                    # Erase EVERYTHING in the phone body completely (removes all gradients, logo, shadows)
                    pixels[x, y] = (0, 0, 0, 0)
                else:
                    # Inside the camera box, only erase the grey phone background between the lenses
                    r, g, b, a = pixels[x, y]
                    if 40 <= r <= 100 and 40 <= g <= 100 and 40 <= b <= 100:
                        pixels[x, y] = (0, 0, 0, 0)
...
if bbox:
    ...
    print("Successfully generated clean transparent Samsung Ultra template!")
else:
